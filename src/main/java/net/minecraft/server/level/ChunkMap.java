...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   private static boolean isChunkInEuclideanRange(ChunkPos p_183790_, ServerPlayer p_183791_, boolean p_183792_, int p_183793_) {
      int i;
      int j;
      if (p_183792_) {
         SectionPos sectionpos = p_183791_.getLastSectionPos();
         i = sectionpos.x();
         j = sectionpos.z();
      } else {
         i = SectionPos.blockToSectionCoord(p_183791_.getBlockX());
         j = SectionPos.blockToSectionCoord(p_183791_.getBlockZ());
      }

      return isChunkInEuclideanRange(p_183790_, i, j, p_183793_);
   }

   private static boolean isChunkInEuclideanRange(ChunkPos p_183765_, int p_183766_, int p_183767_, int p_183768_) {
      int i = p_183765_.x;
      int j = p_183765_.z;
      return isChunkInEuclideanRange(i, j, p_183766_, p_183767_, p_183768_);
   }

   public static boolean isChunkInEuclideanRange(int p_183721_, int p_183722_, int p_183723_, int p_183724_, int p_183725_) {
      int i = p_183721_ - p_183723_;
      int j = p_183722_ - p_183724_;
      int k = p_183725_ * p_183725_ + p_183725_;
      int l = i * i + j * j;
      return l <= k;
   }

   private static boolean isChunkOnEuclideanBorder(ChunkPos p_183841_, ServerPlayer p_183842_, boolean p_183843_, int p_183844_) {
      int i;
      int j;
      if (p_183843_) {
         SectionPos sectionpos = p_183842_.getLastSectionPos();
         i = sectionpos.x();
         j = sectionpos.z();
      } else {
         i = SectionPos.blockToSectionCoord(p_183842_.getBlockX());
         j = SectionPos.blockToSectionCoord(p_183842_.getBlockZ());
      }

      return isChunkOnEuclideanBorder(p_183841_.x, p_183841_.z, i, j, p_183844_);
   }

   private static boolean isChunkOnEuclideanBorder(int p_183829_, int p_183830_, int p_183831_, int p_183832_, int p_183833_) {
      if (!isChunkInEuclideanRange(p_183829_, p_183830_, p_183831_, p_183832_, p_183833_)) {
         return false;
      } else if (!isChunkInEuclideanRange(p_183829_ + 1, p_183830_, p_183831_, p_183832_, p_183833_)) {
         return true;
      } else if (!isChunkInEuclideanRange(p_183829_, p_183830_ + 1, p_183831_, p_183832_, p_183833_)) {
         return true;
      } else if (!isChunkInEuclideanRange(p_183829_ - 1, p_183830_, p_183831_, p_183832_, p_183833_)) {
         return true;
      } else {
         return !isChunkInEuclideanRange(p_183829_, p_183830_ - 1, p_183831_, p_183832_, p_183833_);
      }
   }
      ...
      if (i != this.viewDistance) {
         ...
         for(ChunkHolder chunkholder : this.updatingChunkMap.values()) {
            ...
            this.getPlayers(chunkpos, false).forEach((p_199345_) -> {
               boolean flag = isChunkInEuclideanRange(chunkpos, p_199345_, true, j);
               boolean flag1 = isChunkInEuclideanRange(chunkpos, p_199345_, true, this.viewDistance);
   ...
   void updatePlayerStatus(ServerPlayer p_140193_, boolean p_140194_) {
      ...
      for(int l = i - this.viewDistance; l <= i + this.viewDistance; ++l) {
         for(int k = j - this.viewDistance; k <= j + this.viewDistance; ++k) {
            if (isChunkInEuclideanRange(l, k, i, j, this.viewDistance)) {
      ...
      if (Math.abs(k - l1) <= this.viewDistance * 2 && Math.abs(l - i2) <= this.viewDistance * 2) {
         ...
         for(int l3 = k2; l3 <= j3; ++l3) {
            for(int k1 = i3; k1 <= k3; ++k1) {
               ...
               boolean flag5 = isChunkInEuclideanRange(chunkpos1, k, l, this.viewDistance);
               boolean flag6 = isChunkInEuclideanRange(chunkpos1, l1, i2, this.viewDistance);
               this.updateChunkTracking(p_140185_, chunkpos1, new MutableObject<>(), flag5, flag6);
            }
         }
      } else {
         for(int i1 = k - this.viewDistance; i1 <= k + this.viewDistance; ++i1) {
            for(int j1 = l - this.viewDistance; j1 <= l + this.viewDistance; ++j1) {
               if (isChunkInEuclideanRange(i1, j1, k, l, this.viewDistance)) {
         ...
         for(int j2 = l1 - this.viewDistance; j2 <= l1 + this.viewDistance; ++j2) {
            for(int l2 = i2 - this.viewDistance; l2 <= i2 + this.viewDistance; ++l2) {
               if (isChunkInEuclideanRange(j2, l2, l1, i2, this.viewDistance)) {
                  ChunkPos chunkpos2 = new ChunkPos(j2, l2);
                  boolean flag7 = false;
                  boolean flag8 = true;
                  this.updateChunkTracking(p_140185_, chunkpos2, new MutableObject<>(), false, true);
               }
            }
         }
      }
   ...
   public List<ServerPlayer> getPlayers(ChunkPos p_183801_, boolean p_183802_) {
      ...
      for(ServerPlayer serverplayer : set) {
         if (p_183802_ && isChunkOnEuclideanBorder(p_183801_, serverplayer, true, this.viewDistance) || !p_183802_ && isChunkInEuclideanRange(p_183801_, serverplayer, true, this.viewDistance)) {
            builder.add(serverplayer);
         }
      }
